...
from torch_geometric_temporal.nn.recurrent import TGCN, AGCRN, GCLSTM
...
class IGNNK(nn.Module):
  ...
  x^y_m
    ...
    def forward(self, x, A_q, A_h, A):
        ...
        edge_index = edge_index.view(2, 1, -1).repeat(1, batch_size, 1) + torch.arange(batch_size).view(1, -1, 1).to(torch.device("cuda:0")) * num_nodes
        ...
        for i in range(self.time_dimension):
            y_hat, h, c = self.recurrent(X_S[:, i, None], edge_index, edge_weight, h, c)


        self.recurrent(X_S.view(X_S.shape[0], 1, X_S.shape[1]), edge_index, edge_weight)
